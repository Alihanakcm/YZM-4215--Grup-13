...
namespace WebUI.Filters
{
    public class LoginFilter : IActionFilter 
    {
        public void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (result == null)
            {
                context.Result = controller.RedirectToAction("AdminLogin", "Account");
            }
            else
            {
        ...
        public string IsRefreshToken(byte[] sessionToken, ActionExecutingContext context)
        {
            ...
            {
                string tokenn = Guid.NewGuid().ToString() + "æ" + DateTime.Now;
                context.HttpContext.Session.Set("tokenn", System.Text.Encoding.UTF8.GetBytes(tokenn));
                return tokenn;
            }
            return string.Empty;
        }
